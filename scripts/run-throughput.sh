...
start_job() {
	...
	PARA=$(($1*$slots))
	"${FLINK_BIN}" run -m yarn-cluster -yn $1 -yst -yD taskmanager.network.credit-based-flow-control.enabled=$CREDIT_BASED_ENABLED -yjm 768 -ytm 3072 -ys $slots -yd -p $PARA -c "$CLASS" "${JOB_JAR}" --ft $CHECKPOINTING_INTERVAL --sleepFreq $SOURCE_DELAY_FREQ --repartitions $REPART --timeout $BUFFER_TIMEOUT --payload $PAYLOAD_SIZE --delay $SOURCE_DELAY --logfreq $LOG_FREQ --latencyFreq $LATENCY_MEASURE_FREQ | tee lastJobOutput
}
